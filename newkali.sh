...
echo -e "${CYA}[3] Exit"
echo -e "${YEL}[4] Manual run custom bash script${NC}"
echo
...
if [ "$choice" == "1" ]; then
    ...
    echo
    echo -e "${YEL}########################################${NC}"
    echo -e "${BLU}   ✅ Done! Script by Yatharth ✅ ${NC}"
    echo -e "${YEL}########################################${NC}"
    ...
    echo -e "${YEL}Enter path to your custom bash script:${NC}"
    read -p "> " custom_script
    if [ -f "$custom_script" ]; then
        echo -e "${CYA}[*] Running your script...${NC}"
        bash "$custom_script"
    else
